  ...
  <div ref="canvasBodyRef" class="canvas-body" @click.self="store.clearSelection()">
    <div class="canvas-stack">
      ...
      >
        ...
        <MultiSelectToolbar
          v-if="store.activeSectionId === section.id && store.selectedIds.length >= 2"
...
const onMarqueeStart = (payload: { sectionId: string; x: number; y: number }) => {
...
const onGlobalMouseUp = (e: MouseEvent) => {
  ...
  const sectionEl = canvasBody.querySelector(`[data-section-id="${marquee.sectionId}"]`) as HTMLElement | null
  if (!sectionEl) return
...
const onKeyDown = (e: KeyboardEvent) => {
  ...
  if ((e.key === 'Delete' || e.key === 'Backspace') && selected.length > 0) {
    e.preventDefault()
    selected.forEach((id) => store.removeElement(id))
    return
  }
